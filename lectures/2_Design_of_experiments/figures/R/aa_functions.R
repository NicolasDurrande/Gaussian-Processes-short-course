...
tangocol <- c(darkBlue, darkRed, darkGreen, darkChocolate , darkOrange, darkButter, darkPurple)

branin <- function(x){
  x1 <- x[,1] * 15 - 5
  x2 <- x[,2] * 15
  z <- (x2 - 5/(4 * pi^2) * (x1^2) + 5/pi * x1 - 6)^2 + 10 * (1 - 1/(8 * pi)) * cos(x1) + 10
  return(z)
}
...
kBrown <- function(x,y,param){
  param[1]*outer(x[,1],y[,1],"pmin")
}

kLinear <- function(x,y,param){
  param[1]*outer(x[,1],y[,1])
}
...
kConst <- function(x,y,param){
  matrix(param[1],dim(x)[1],dim(y)[1])
}

kGauss <- function(x,y,param=NULL){
  if(is.null(param)) param <- c(1,rep(0.2,ncol(x)))
...
GPR <- function(x,X,F,kern,param,kernNoise=kWhite,paramNoise=0){
  K_1 <- solve(kern(X,X,param) + kWhite(X,X,paramNoise))
  m <- kern(x,X,param)%*%K_1%*%F
  K <- kern(x,x,param) - kern(x,X,param)%*%K_1%*%kern(X,x,param)  
