...
x = np.random.uniform(np.min(Xnew01,0),np.max(Xnew01,0),(10000,4))
ei = EIn(x,mopt)
np.max(ei)
...
pb.subplot(221)
pb.plot(Xs[:,0:1],-Fs,'kx',mew=1.5)
pb.axvline(xopt[0],color='g')
pb.xlabel(namesNew[0]), pb.ylabel('flight duration')

pb.subplot(222)
pb.plot(Xs[:,1:2],-Fs,'kx',mew=1.5)
pb.axvline(xopt[1],color='g')
pb.xlabel(namesNew[1]), pb.ylabel('flight duration')

pb.subplot(223)
pb.plot(Xs[:,2:3],-Fs,'kx',mew=1.5)
pb.axvline(xopt[2],color='g')
pb.xlabel(namesNew[2]), pb.ylabel('flight duration')

pb.subplot(224)
pb.plot(Xs[:,3:4],-Fs,'kx',mew=1.5)
pb.axvline(xopt[3],color='g')
pb.xlabel(namesNew[3]), pb.ylabel('flight duration')
